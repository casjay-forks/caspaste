...
TEMP_DIR=$(mktemp -d /tmp/caspaste-test-XXXXXX)
...
if docker run --rm \
    ...
    -e GOCACHE=/go/build \
    -e GOMODCACHE=/go/pkg/mod \
    golang:alpine sh -c 'go build -trimpath -tags netgo -ldflags "-w -s -X main.Version=test" -o /out/caspaste ./src/cmd/caspaste' 2>&1; then
...
if docker run --rm \
    ...
    -e GOCACHE=/go/build \
    -e GOMODCACHE=/go/pkg/mod \
    golang:alpine sh -c 'go build -trimpath -tags netgo -ldflags "-w -s -X main.Version=test" -o /out/caspaste-cli ./src/cmd/caspaste-cli' 2>&1; then
